...
******************************************************************************/
FvsMinutiaSet_t MinutiaSetCreate(const FvsInt_t size,char *Personno,int Fingerno) {
...
std::string GetMinutiaBuffer(const FvsMinutiaSet_t min)
{
	std::string ss = "";
	iFvsMinutiaSet_t* minutia = (iFvsMinutiaSet_t*)min;
	FvsError_t nRet = FvsOK;
	if (minutia != NULL)
	{
		ss += minutia->personNo;
		ss +="\n";
		ss += std::to_string(minutia->finger) + "\t";
		ss += std::to_string(minutia->nbminutia) ;
		ss += "\n";
		for (int i = 0; i < minutia->nbminutia; i++)
		{
			ss += std::to_string(minutia->ptable[i].x) + "\t";
			ss += std::to_string(minutia->ptable[i].y) + "\t";
			ss += std::to_string(minutia->ptable[i].angle) + "\t";
			ss += std::to_string(minutia->ptable[i].type) + "\n";
